/**
 * Hook used to check the health status of the UPRN download service. It manages the loading state,
 * any errors that occur during the health check, and the resolved health status.
 * @param url The URL to the UPRN download health endpoint.
 * @returns The loading, error, content states as well as a fetch method.
 */
export function useUprnDownloadHealth(url: string) {
