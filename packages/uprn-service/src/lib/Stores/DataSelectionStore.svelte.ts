...
export class DataSelectionStore {
	 ...
	 *
	 * Stores the provided DataSelection keyed by its nodeId and shows a
	...
	public addSelection(dataSelection: DataSelectionSnapshot) {
		this.dataSelections.set(dataSelection.nodeId, dataSelection);
	}

	/**
	 * Update an existing DataSelection in the store.
	 * @param nodeId - The id of the layer whose selection should be updated.
	 * @param selectedFieldIds - The new array of selected field IDs.
	 */
	public updateSelection(nodeId: string, selectedFieldIds: string[]) {
		const existing = this.dataSelections.get(nodeId);
		if (!existing) {
			console.warn(`[data-selection-store] no existing selection for nodeId ${nodeId}`);
			return;
		}

		existing.selectedFieldIds = new SvelteSet(selectedFieldIds);
		this.dataSelections.set(nodeId, { ...existing });
	}

	public addOrUpdateSelection(nodeId: string, selectedFieldIds: string[]) {
		const existing = this.dataSelections.get(nodeId);
		if (existing) {
			existing.selectedFieldIds = new SvelteSet(selectedFieldIds);
			this.dataSelections.set(nodeId, { ...existing });
			...
			this.dataSelections.set(nodeId, newSelection);
		}
	}

	/**
	 * Remove a DataSelection from the store by node id.
	 *
	 ...
	 *
	 * @param nodeId - The id of the layer whose selection should be removed.
	 */
	public removeSelection(nodeId: string) {
		console.log(`[data-selection-store] Removing selection for nodeId: ${nodeId}`);
		this.dataSelections.delete(nodeId);
	}

	/**
	 * Get the DataSelection for a given node id.
	 *
	 * @param nodeId - The id of the layer to retrieve the selection for.
	 * @returns The DataSelection if it exists, otherwise undefined.
	 */
	public getSelection(nodeId: string): DataSelectionSnapshot | undefined {
		return this.dataSelections.get(nodeId);
	}
