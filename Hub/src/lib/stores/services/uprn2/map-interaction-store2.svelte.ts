 ...
 */
export class MapInteractionStore2 {
	...
	private setupClickHandler(view: MapView): void {
		this.clickHandle = view.on('click', async (event) => {
			...
			try {
				...
				console.log(
					'[map-interaction-store] Clicked area ID:',
					objectId,
					'Existing handle:',
					existingHandle
				);
				if (existingHandle) {
					...
					console.log('[map-interaction-store] Area deselected:', objectId);
				} else {
					...
					console.log('[map-interaction-store] Area selected:', objectId);
				}
			} catch (error) {
