<script lang="ts">
	...
	import { getAiWhereToBuildConfig } from '$lib/services/apps/ai-where-to-build/ai-where-to-build-config-provider';
	import {
			...
			content: [
				{
					...
					creator: (event: __esri.PopupTemplateCreatorEvent) => {
						...
						const titles: string[] = attrs.layerTitles ?? [];
						const values: string[] = attrs.layerValues ?? [];
						const weights: Array<number | string> = attrs.layerWeights ?? [];

						// aggregated buffer info from buildMergedAttributes
						const bufferInfos =
							(attrs.bufferInfos as
								| Array<{
										distance?: number;
										unit?: string;
										weight?: number;
						...
						titles.forEach((title, idx) => {
							...
							rows.push({
								...
								isBuffer: bufferInfos[idx] ? 'Yes' : 'No',
								bufferDistance:
									bufferInfos[idx] && bufferInfos[idx].distance != null
										? `${bufferInfos[idx].distance} ${bufferInfos[idx].unit ?? ''}`.trim()
										: ''
							});
						});

						// ----------------- DETERMINE WHICH COLUMNS TO SHOW -----------------
						const hasValueCol = rows.some((r) => r.value && r.value.trim() !== '');
						const hasWeightCol = rows.some((r) => r.weight && r.weight.trim() !== '');
						const hasIsBufferCol = rows.some((r) => r.isBuffer && r.isBuffer.trim() !== '');
						const hasBufferDistanceCol = rows.some(
							(r) => r.bufferDistance && r.bufferDistance.trim() !== ''
						);
						...
						let thValue: HTMLTableCellElement | undefined;
						let thWeight: HTMLTableCellElement | undefined;
						let thIsBuffer: HTMLTableCellElement | undefined;
						let thBufferDist: HTMLTableCellElement | undefined;

						thLayer.style.whiteSpace = 'nowrap';
						thLayer.style.wordBreak = 'normal';

						if (hasValueCol) {
							thValue = document.createElement('th');
							thValue.textContent = 'Value';
							thValue.style.whiteSpace = 'nowrap';
							thValue.style.wordBreak = 'normal';
							headerRow.appendChild(thValue);
						}

						if (hasWeightCol) {
							thWeight = document.createElement('th');
							thWeight.textContent = 'Weight';
							thWeight.style.whiteSpace = 'nowrap';
							thWeight.style.wordBreak = 'normal';
							headerRow.appendChild(thWeight);
						}

						if (hasIsBufferCol) {
							thIsBuffer = document.createElement('th');
							thIsBuffer.textContent = 'Is buffer';
							thIsBuffer.style.whiteSpace = 'nowrap';
							thIsBuffer.style.wordBreak = 'normal';
							headerRow.appendChild(thIsBuffer);
						}

						if (hasBufferDistanceCol) {
							thBufferDist = document.createElement('th');
							thBufferDist.textContent = 'Buffer distance';
							thBufferDist.style.whiteSpace = 'nowrap';
							thBufferDist.style.wordBreak = 'normal';
							headerRow.appendChild(thBufferDist);
						}
						...
						rows.forEach((row) => {
							...
							if (hasWeightCol) {
								...
								tdWeight.style.whiteSpace = 'nowrap';
								tdWeight.style.wordBreak = 'normal';
								tr.appendChild(tdWeight);
							}

							// isBuffer
							if (hasIsBufferCol) {
								const tdIsBuffer = document.createElement('td');
								tdIsBuffer.textContent = row.isBuffer;
								tdIsBuffer.style.whiteSpace = 'nowrap';
								tdIsBuffer.style.wordBreak = 'normal';
								tr.appendChild(tdIsBuffer);
							}

							// bufferDistance
							if (hasBufferDistanceCol) {
								const tdBufferDist = document.createElement('td');
								tdBufferDist.textContent = row.bufferDistance;
								tdBufferDist.style.whiteSpace = 'nowrap';
								tdBufferDist.style.wordBreak = 'normal';
								tr.appendChild(tdBufferDist);
							}
	...
	async function onAnalyze() {
		...
		for (const layerSetting of config.analysisSettings as LayerAnalysisSettings[]) {
			...
			if (layer.geometryType === 'polygon') {
				await processPolygonFeatureLayer(parcelPolygon, layer as __esri.FeatureLayer, layerSetting);
